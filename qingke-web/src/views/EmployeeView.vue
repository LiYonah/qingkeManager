...
const modalFields = [
  ...
  { key:'salary', label:'月薪', type:'number', required:true },
  { key:'status', label:'状态', type:'select', options:[{value:1,label:'在职'},{value:0,label:'离职'}] }
]

async function load() {
  loading.value = true
  try {
    const res = await getEmployees({ page: page.value, size: 10, keyword: keyword.value })
...
function openCreate() {
  editId.value = null; modalTitle.value = '新增员工'; modalInitial.value = null; modalVisible.value = true
}
function openEdit(row) {
  editId.value = row.id; modalTitle.value = '编辑员工'; modalInitial.value = row; modalVisible.value = true
}
async function handleDelete(row) {
  if (!confirm('确定删除 ' + row.name + ' 吗？')) return
  await deleteEmployee(row.id); load()
}
async function handleSubmit(data) {
  if (data.salary !== null) data.salary = Number(data.salary)
  if (data.status !== null) data.status = Number(data.status)
  if (editId.value) { await updateEmployee(editId.value, data) }
  else { await createEmployee(data) }
  modalVisible.value = false; load()
}

const statusBadge = (s) => s === 1 ? 'badge-success' : 'badge-neutral'
const statusText = (s) => s === 1 ? '在职' : s === 0 ? '离职' : '—'
</script>

<template>
  <div>
    <div class="page-header">
      <div><h3>👥 员工管理</h3><p class="crumb">员工列表</p></div>
      <button class="btn-add" @click="openCreate">+ 新增员工</button>
    </div>
    <div class="controls">
      <input v-model="keyword" placeholder="搜索姓名..." @keyup.enter="search" class="search-input" />
      <button @click="search" class="search-btn">搜索</button>
    </div>
    <div class="table-wrap">
      <table v-if="list.length">
        <thead><tr><th>ID</th><th>姓名</th><th>部门</th><th>职位</th><th>薪资</th><th>状态</th><th>操作</th></tr></thead>
        <tbody>
          <tr v-for="row in list" :key="row.id">
            <td>{{ row.id }}</td><td>{{ row.name }}</td>
            <td>{{ row.department || '—' }}</td><td>{{ row.position || '—' }}</td>
            <td>{{ row.salary ? '¥' + Number(row.salary).toFixed(2) : '—' }}</td>
            <td><span :class="['badge', statusBadge(row.status)]">{{ statusText(row.status) }}</span></td>
            <td class="actions">
              <button class="act-edit" @click="openEdit(row)">编辑</button>
              <button class="act-del" @click="handleDelete(row)">删除</button>
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <FormModal :visible="modalVisible" :title="modalTitle" :fields="modalFields"
      :initial="modalInitial" @close="modalVisible=false" @submit="handleSubmit" />
  </div>
</template>
